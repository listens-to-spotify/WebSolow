export class SolowModel {
    ...
    dkdt(k: number): number {
        return this.sY(k) - this.amortization(k);
    }
    ...
    amortizationLine(kMin = 0, kMax = 5, points = 100): { x: number; y: number }[] {
        const data = [];
        const step = (kMax - kMin) / points;
        for (let k = kMin; k <= kMax; k += step) {
            data.push({ x: k, y: this.amortization(k) });
        }
        return data;
    }
    ...
    simulateTraj(steps = 500): TrajectoryPoint[] {
        // return this.simulateRK4();
    ...
    createPoint(t: number, k: number): TrajectoryPoint {
        return {
            ...
            amortization: this.amortization(k),
            dkdt: this.dkdt(k),
            gw: this.gw(k),
            gy: this.gy(k),
            r: this.r(k)
